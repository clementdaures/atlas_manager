...
from atlas_manager.ui.Qt import QtWidgets, QtCompat
from atlas_manager.ui.main import MainUI as atlas_main
from atlas_manager.dcc.main_core import MainCore
from atlas_manager.dcc.maya import utils
from atlas_manager.dcc.maya import panels
from atlas_manager.dcc.maya import validate
from atlas_manager.dcc.maya import extract
from atlas_manager.dcc.maya import ingest
from atlas_manager.dcc.maya import extension
...
class Dcc(MainCore):
    ...
    def launch(self, atlas_main_object, window_name=None, dont_show=False):
        """Launch Atlas Main UI with DCC specific way."""
        ...
        if not dont_show:
            main_ui = DockableMainUI(main_object=atlas_main_object, window_name=window_name, parent=parent)
            ...
            for entry in all_widgets:
                try:
                    if entry.objectName() == window_name+"NoShow":
                        # if entry.objectName().startswith("Atlas Manager"):
                        entry.close()
                        entry.deleteLater()
                except (AttributeError, TypeError):
                    pass
            main_ui = RegularMainUI(main_object=atlas_main_object, parent=parent, window_name=window_name+"NoShow")

        return main_ui


class RegularMainUI(atlas_main):
...
class DockableMainUI(MayaQWidgetDockableMixin, atlas_main):
    def __init__(self, main_object=None, window_name=None, parent=None, **kwargs):
