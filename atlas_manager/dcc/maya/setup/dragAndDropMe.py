...
def onMayaDroppedPythonFile(*args, **kwargs):
    ...
    if sys.version_info.major < 3:
        cmds.confirmDialog(title='ERROR:', message="Atlas Manager requires Python version 3.6 and higher. Current Maya Python interpreter is not compatible. \n\nAborting.", button=['OK'],
                       defaultButton='OK')
        return
    _add_module()
...
def _add_module():
    trigger_module = os.path.normpath(
        os.path.join(os.path.dirname(__file__), "atlas_manager_module")
    )
    module_file_content = """+ atlas_manager 4.0.1 %s""" % trigger_module
    ...
    user_module_file = os.path.join(user_module_dir, "atlas_manager.mod")
    ...
    cmds.confirmDialog(
        title="Atlas Manager",
        message="Atlas Manager Installed. Please restart Maya to see the shelf and menu items.",
    )
    pass
