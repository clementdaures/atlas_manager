...
def add_python_path():
    ...
    atlas_manager_path = os.path.abspath(os.path.join(dcc_path, os.pardir))
    home_path = os.path.abspath(os.path.join(atlas_manager_path, os.pardir))
...
def load_menu():
    main_ui_command = "from atlas_manager.ui import main as atlas_main\natlas_main.launch(dcc='Maya')"
    add_to_menu("AtlasManager", "Main UI", main_ui_command)

    new_version_command = "from atlas_manager.ui import main\ntui = main.launch('Maya', dont_show=True)\ntui.on_new_version()"
    add_to_menu("AtlasManager", "New Version", new_version_command)

    publish_scene_command = "from atlas_manager.ui import main\ntui = main.launch('Maya', dont_show=True)\ntui.on_publish_scene()"
    add_to_menu("AtlasManager", "Publish Scene", publish_scene_command)

    # add a separator
    cmds.menuItem(divider=True)
    recreate_command = "atlas_manager_setup.load_shelves(reset=True)"
    add_to_menu("AtlasManager", "Re-create Shelves", recreate_command)
...
def add_to_menu(menu, menu_item, command):
    ...
    if cmds.menu(menu_widget, label=menu, exists=True, parent=main_window):
        atlas_manager_menu = '%s|%s' % (main_window, menu_widget)
    else:
        atlas_manager_menu = cmds.menu(menu_widget, label=menu, parent=main_window, tearOff=True)
