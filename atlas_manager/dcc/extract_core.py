...
import importlib
from atlas_manager.external import fileseq
from atlas_manager.core import filelog
from atlas_manager.core.settings import Settings
from atlas_manager.objects.metadata import Metadata
...
class ExtractCore:
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        cls.name = name_map.get(module_name, module_name)
    ...
    def resolve_output(self):
        ...
        if self.bundled:
            output_path = (
                Path(self.extract_folder) / f"{self.name}_{self._extract_name}_{self.version_string}" #.upper()
            )
        else:
            output_path = (
                Path(self.extract_folder)
                / f"{self.name}_{self._extract_name}_{self.version_string}{self.extension}" #.upper()
            )
