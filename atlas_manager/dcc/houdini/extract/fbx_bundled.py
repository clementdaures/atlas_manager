"""Extract Fbx bundle from Houdini scene"""
...
class FbxBundled(ExtractCore):
    """Extract multiple Fbx from Houdini scene into a bundle"""

    nice_name = "Fbx Bundled"
    color = (180, 255, 100)
    bundled = True
    optional = True
    ...
    def __init__(self):
        ...
        # Find all ROP Fbx nodes in the scene
        rop_fbx_nodes = []
        for node in hou.node("/").allSubChildren():
            if node.type().name() == "rop_fbx":
                rop_fbx_nodes.append(node)

        # Get ROP names for the UI list
        rop_items = [node.path() for node in rop_fbx_nodes]
        ...
        global_exposed_settings = {
            ...
            "rop_nodes": {
                "display_name": "ROP Fbx Nodes to Export",
                "type": "list",
                "value": rop_items,
            },
        }
        ...
        if hou.isApprentice():
            msg = "Fbx export is not supported in Houdini Apprentice."
            LOG.error(msg)
            self._message = msg
            self._enabled = False
    ...
    def _extract_default(self):
        """Bundle multiple Fbx from existing ROP Fbx nodes"""
        ...
        for rop_path in selected_rop_paths:
            ...
            if rop_node.type().name() != "rop_fbx":
                LOG.warning(f"Node {rop_path} is not a ROP Fbx node, skipping...")
                continue
