...
class Usd(IngestCore):
    """Ingest USD files as sublayers."""

    nice_name = "USD Sublayer"
    ...
    def _find_existing_sublayer(self, node_name):
        """Find existing sublayer node, prioritizing current network."""
    ...
    def _create_or_update_sublayer(self, parent, file_path, node_name):
        """Create or update sublayer node."""
        # Check if node already exists in parent
        sublayer_node = parent.node(node_name)

        if sublayer_node:
            LOG.info(f"Updating existing sublayer node '{node_name}'")
        else:
            sublayer_node = parent.createNode("sublayer", node_name)
            LOG.info(f"Created sublayer node '{node_name}'")
            sublayer_node.moveToGoodPosition()

        sublayer_node.parm("filepath1").set(file_path)

        LOG.info(f"Set file path: {file_path}")
        return sublayer_node
    ...
    def _bring_in_default(self):
        ...
        existing_node = self._find_existing_sublayer(node_name)
        ...
        if current_network and current_network.type().category().name() == "Lop":
            LOG.info(f"Creating sublayer in current network: {current_network.path()}")
            return self._create_or_update_sublayer(current_network, file_path, node_name)
        else:
            # Fallback to default locations
            asset = hou.node("/stage/ASSET")
            if asset:
                return self._create_or_update_sublayer(asset, file_path, node_name)
            else:
                LOG.warning("/stage/ASSET not found, using /stage")
                lopnet = self._get_or_create_lopnet("/stage")
                return self._create_or_update_sublayer(lopnet, file_path, node_name)

    def _reference_default(self):
        """Reference USD file as sublayer in /stage."""
