...
from atlas_manager.core.constants import DataTypes, BranchingModes

from atlas_manager.ui.Qt import QtWidgets, QtCore
from atlas_manager.ui.widgets.validated_string import ValidatedString
from atlas_manager.ui.widgets.settings_widgets import (
    UsersDefinitions,
    MetadataDefinitions,
    CategoryDefinitions
)
from atlas_manager.ui.widgets.switch_tree import SwitchTreeWidget, SwitchTreeItem
from atlas_manager.ui.widgets.common import (
    HeaderLabel,
    ResolvedText,
    AtlasButtonBox,
    AtlasButton,
...
from atlas_manager.ui.layouts.settings_layout import (
    SettingsLayout,
    convert_to_ui_definition,
    convert_to_settings_data,
)
from atlas_manager.ui.dialog.data_containers import MainLayout
...
class SettingsDialog(QtWidgets.QDialog):
    ...
    def build_static_widgets(self):
        ...
        atlas_button_box = AtlasButtonBox(parent=self)
        self.layouts.buttons_layout.addWidget(atlas_button_box)
        self.apply_button = atlas_button_box.addButton(
            "Apply", QtWidgets.QDialogButtonBox.ApplyRole
        )
        self.apply_button.setEnabled(False)
        cancel_button = atlas_button_box.addButton(
            "Cancel", QtWidgets.QDialogButtonBox.RejectRole
        )
        ok_button = atlas_button_box.addButton(
    ...
    def __create_user_password_layout(self):
        ...
        header_layout.addWidget(HorizontalSeparator(color=(122, 83, 207), height=1))
        ...
        change_password_button = AtlasButton(text="Change Password", parent=self)
    ...
    def common_title(self):
        ...
        # ui_definition = management.platforms["tractor"].get_settings_ui()
    ...
    def _gather_validations_and_extracts(self):
        ...
        if not is_frozen:
            # get the location of the file. atlas_manager/ui/dialog/settings_dialog.py
            _file_path = Path(__file__)
            atlas_manager_path = _file_path.parents[2]
        else:
            # First get the location of the executable
            # which is in by default atlas_manager/dist/atlas/<name>.exe
            _exe_path = Path(sys.executable)
            # Pick walk up to the atlas_manager folder. This will be different if the executable is somewhere else.
            atlas_manager_path = _exe_path.parents[2]

        # DCC folder
        _dcc_folder = atlas_manager_path / "dcc"
...
if __name__ == "__main__":
    ...
    atlas = atlas_manager.initialize("Standalone")
    _style_file = pick.style_file()
    dialog = SettingsDialog(atlas, styleSheet=str(_style_file.readAll(), "utf-8"))
    dialog.show()
    sys.exit(app.exec_())
