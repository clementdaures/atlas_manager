...
class TaskDialog(QtWidgets.QDialog):
    ...
    def _get_metadata_type(self, data):
        ...
        enums = data.get("enum", [])
        data_type = atlas_manager.ui.layouts.settings_layout.guess_data_type(
            default_value, enums
        )
        if not data_type:
            raise ValueError(
    ...
    def _init_ui(self):
        ...
        self.settings_layout = atlas_manager.ui.layouts.settings_layout.SettingsLayout(
        ...
        secondary_content = atlas_manager.ui.layouts.settings_layout.SettingsLayout(
            secondary_definition, self.secondary_data, parent=self
        )
        self.secondary_layout.contents_layout.addLayout(secondary_content)
        tertiary_content = atlas_manager.ui.layouts.settings_layout.SettingsLayout(
...
class NewTask(TaskDialog):
    ...
    def execute(self):
        ...
        for sub in self._parent_sub:
            _new_task = self.atlas_project.create_task(
            ...
            if _new_task == -1:
                self.feedback.pop_info(
                    ...
                    text=self.atlas_project.log.last_message,
                    critical=True,
                )
                return
