...
from atlas_manager.ui.widgets.common import HeaderLabel, ResolvedText
...
class NewWorkDialog(QtWidgets.QDialog):
    ...
    def build_ui(self):
        ...
        self.widgets.header_lbl.set_color("#FBFFFF")
        ...
        self.widgets.resolved_name_lbl.set_color("#9688FF")
        ...
        # create a the AtlasButtonBox
        button_box = AtlasButtonBox(parent=self)
...
class NewVersionDialog(QtWidgets.QDialog):
    ...
    def build_ui(self):
        ...
        _color = "#FBFFFF"  # "orange" if not self.ingest else "pink"
        ...
        self.name_label.set_color("rgb(122, 83, 207)")
...
class SaveAnyFileDialog(NewWorkDialog):
    """Dialog to save any file or folder to the atlas manager project."""
...
if __name__ == "__main__":
    ...
    atlas.user.set("Admin", "1234")
    # atlas.user.set("Generic", "1234")
    sub = atlas.project.find_sub_by_path("Assets/Characters/Soldier")
    sub.scan_tasks()
    task = sub.tasks.get("bizarro")
    category = task.categories.get("Model")
    works = category.scan_works()
    work = list(works.values())[0]
    dialog = NewWorkDialog(main_object=atlas)
