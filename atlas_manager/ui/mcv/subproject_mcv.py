...
import atlas_manager.ui.dialog.subproject_dialog
import atlas_manager.ui.dialog.task_dialog
...
class AtlasSubItem(QtGui.QStandardItem):
    def __init__(self, sub_obj):
        super(AtlasSubItem, self).__init__()
...
class AtlasColumnItem(QtGui.QStandardItem):
    def __init__(self, name, overridden=False):
        super(AtlasColumnItem, self).__init__()
...
class AtlasSubModel(QtGui.QStandardItemModel):
    def __init__(self, structure_object):
        super(AtlasSubModel, self).__init__()
    ...
    def populate(self):
        ...
        parent_row = self
        self.root_item = AtlasSubItem(self.project)
    ...
    def append_sub(self, sub_obj, parent):
        _sub_item = AtlasSubItem(sub_obj)
        # generate the column texts
        # name, id and path are mandatory, start with them
        _row = [_sub_item, AtlasColumnItem(str(sub_obj.id)), AtlasColumnItem(sub_obj.path)]
        for column in self.columns[3:]:  # skip the first 3 columns which are mandatory
            ...
            _column_item = AtlasColumnItem(str(_column_value), _overridden)
    ...
    def update_item(self, item, sub_obj):
        ...
        for index, column in enumerate(self.columns):
            ...
            if isinstance(_column_item, AtlasColumnItem):
                _column_value = sub_obj.metadata.get_value(column, "")
                _overridden = sub_obj.metadata.is_overridden(column)
...
class AtlasSubView(QtWidgets.QTreeView):
    ...
    def selectionChanged(self, *args, **kwargs):
        super(AtlasSubView, self).selectionChanged(*args, **kwargs)
        self.get_tasks()
    ...
    def expandAll(self):
        super(AtlasSubView, self).expandAll()
    ...
    def collect_tasks(sub_items, recursive=True, filtered=True):
        ...
        for sub_item in sub_items:
            if not isinstance(sub_item, atlas_manager.objects.subproject.Subproject):
    ...
    def set_project(self, project_obj):
        self.model = AtlasSubModel(project_obj)
    ...
    def new_sub_project(self, item):
        ...
        if self.model.project.check_permissions(level=2) == -1:
            ...
            return
        _dialog = atlas_manager.ui.dialog.subproject_dialog.NewSubprojectDialog(
    ...
    def edit_sub_project(self, item):
        ...
        if self.model.project.check_permissions(level=2) == -1:
            ...
            self._feedback.pop_info(title.capitalize(), message)
            return
        _dialog = atlas_manager.ui.dialog.subproject_dialog.EditSubprojectDialog(
            self.model.project, parent_sub=item.subproject, parent=self,
    ...
    def new_task(self):
        ...
        if self.model.project.check_permissions(level=2) == -1:
            ...
            self._feedback.pop_info(title.capitalize(), message)
            return
        _dialog = atlas_manager.ui.dialog.task_dialog.NewTask(
...
class AtlasSubProjectLayout(QtWidgets.QVBoxLayout):
    def __init__(self, project_obj, recursive_enabled=True, right_click_enabled=True):
        ...
        self.refresh_btn = AtlasIconButton(
        ...
        self.sub_view = AtlasSubView(project_obj, right_click_enabled=right_click_enabled)
