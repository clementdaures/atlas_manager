...
from atlas_manager.core.constants import ObjectType, ValidationResult, ValidationState
from atlas_manager.ui.Qt import QtWidgets, QtCore, QtGui
from atlas_manager.ui.dialog.feedback import Feedback
from atlas_manager.ui.widgets.common import AtlasButton, HorizontalSeparator, AtlasIconButton
from atlas_manager.ui.widgets.screenshot import take_screen_area
from atlas_manager.ui.widgets.info import ImageWidget, NotesEditor
from atlas_manager.ui.dialog.bunde_ingest_dialog import BundleIngestDialog
from atlas_manager.ui.dialog.info_dialog import InfoDialog
...
class HeaderWidgets:
    label: QtWidgets
    refresh_btn: AtlasIconButton


@dataclass
class VersionWidgets:
    border: QtWidgets.QWidget
    sync_btn: AtlasButton
    lbl: QtWidgets.QLabel
    combo: VersionComboBox
    promote_btn: AtlasIconButton
    preview_btn: AtlasIconButton
    owner_lbl: QtWidgets.QLabel
    info_btn: AtlasIconButton
...
class ElementWidgets:
    ...
    element_view_btn: AtlasIconButton
...
@dataclass
class Buttons:
    import_btn: AtlasButton
    bundle_ingest_btn: AtlasButton
    load_btn: AtlasButton
    reference_btn: AtlasButton


class AtlasVersionLayout(QtWidgets.QVBoxLayout):
    ...
    def __init__(self, project_object, *args, **kwargs):
        """Initialize the AtlasVersionLayout."""
        super().__init__()
        ...
        self.header = HeaderWidgets(
            ...
            refresh_btn=AtlasIconButton(icon_name="refresh", circle=True, size=18, icon_size=14)
        )

        self.version = VersionWidgets(
            border=QtWidgets.QWidget(),
            sync_btn=AtlasButton(text="Sync"),
            lbl=QtWidgets.QLabel(text="Version: "),
            combo=VersionComboBox(),
            promote_btn=AtlasIconButton(icon_name="star.png", circle=False, size=30),
            preview_btn=AtlasIconButton(icon_name="player.png", circle=False, size=30),
            owner_lbl=QtWidgets.QLabel("Owner: "),
            info_btn=AtlasIconButton(icon_name="info.png", circle=False, size=30)
        )

        self.element = ElementWidgets(
            element_lbl=QtWidgets.QLabel("Element: "),
            element_combo=QtWidgets.QComboBox(),
            element_view_btn=AtlasIconButton(icon_name="view.png", circle=False, size=30),
        ...
        self.buttons = Buttons(
            import_btn=AtlasButton("Import"),
            bundle_ingest_btn=AtlasButton("Bundle Ingest"),
            load_btn=AtlasButton("Load"),
            reference_btn=AtlasButton("Reference")
        )
    ...
    def build_ui(self):
        ...
        self.addWidget(HorizontalSeparator(122, 83, 207))
    ...
    def on_replace_thumbnail(self, mode="view"):
        ...
        if mode == "view":
            ...
            temp_file = (Path(tempfile.gettempdir()) /
                         "atlas_manager_screenshot_temp.jpg").as_posix()
