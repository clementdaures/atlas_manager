...
from atlas_manager.core.constants import ObjectType
from atlas_manager.ui.Qt import QtWidgets, QtCore, QtGui
from atlas_manager.ui.dialog.feedback import Feedback
from atlas_manager.ui.dialog.work_dialog import NewVersionDialog
from atlas_manager.ui.widgets.common import HorizontalSeparator, AtlasIconButton
from atlas_manager.ui.widgets.style import ColorKeepingDelegate
from atlas_manager.ui.mcv.filter import FilterModel, FilterWidget

from atlas_manager.ui import pick


class AtlasWorkItem(QtGui.QStandardItem):
    ...
    def __init__(self, work_obj):
        ...
        Args:
            work_obj (atlas_manager.object.work.Work):
                The work object to be represented by this item.
        """
        super(AtlasWorkItem, self).__init__()

        self.atlas_obj = work_obj
        #
        ...
        self.state = None
        self.refresh()
        self.setIcon(pick.icon(self.atlas_obj.dcc.lower()))


    def refresh(self):
        """Refresh the item state."""
        self.set_state(self.atlas_obj.state)
    ...
    def dcc_check(self):
        ...
        return self.atlas_obj.dcc.lower() == self.atlas_obj.guard.dcc.lower()


class AtlasPublishItem(QtGui.QStandardItem):
    ...
    def __init__(self, publish_obj):
        ...
        Args:
            publish_obj (atlas_manager.object.publish.Publish):
                The publish object to be represented by this item.
        """
        super(AtlasPublishItem, self).__init__()

        self.atlas_obj = publish_obj
    ...
    def dcc_check(self):
        """Check if the dcc of the work matches the dcc of the current session."""
        return self.atlas_obj.dcc == self.atlas_obj.guard.dcc


class AtlasCategoryColumnItem(QtGui.QStandardItem):
    ...
    def __init__(self, text):
        ...
        """
        super(AtlasCategoryColumnItem, self).__init__(text)
        self.setEditable(False)


class AtlasCategoryModel(QtGui.QStandardItemModel):
    """Custom QStandardItemModel for the category view."""
    columns = ["name", "id", "path", "creator", "dcc", "date", "version count"]

    def __init__(self):
        """Initialize the model."""
        super(AtlasCategoryModel, self).__init__()
    ...
    def append_publish(self, publish):
        ...
        Args:
            publish (atlas_manager.object.publish.Publish):
                The publish object to be appended to the model.
        Returns:
            AtlasPublishItem: The item that represents the publish
                in the model.
        """
        _item = AtlasPublishItem(publish)
        pid = AtlasCategoryColumnItem(str(publish.publish_id))
        path = AtlasCategoryColumnItem(publish.path)
        creator = AtlasCategoryColumnItem("NA")
        dcc = AtlasCategoryColumnItem(publish.dcc)
        date = AtlasCategoryColumnItem("NA")
        version_count = AtlasCategoryColumnItem(str(publish.version_count))
    ...
    def append_work(self, work):
        ...
        Args:
            work (atlas_manager.object.work.Work):
                The work object to be appended to the model.
        Returns:
            AtlasWorkItem: The item that represents the work
                in the model.
        """
        _item = AtlasWorkItem(work)
        pid = AtlasCategoryColumnItem(str(work.id))
        path = AtlasCategoryColumnItem(work.path)
        creator = AtlasCategoryColumnItem(work.creator)
        dcc = AtlasCategoryColumnItem(work.dcc)
        date = AtlasCategoryColumnItem(
            datetime.fromtimestamp(work.date_modified).strftime("%Y/%m/%d %H:%M:%S")
        )
        version_count = AtlasCategoryColumnItem(str(work.version_count))
...
class AtlasCategoryView(QtWidgets.QTreeView):
    ...
    def __init__(self, parent=None):
        ...
        """
        super(AtlasCategoryView, self).__init__(parent)
        self.purgatory_mode = False
    ...
    def currentChanged(self, *args, **kwargs):
        ...
        """
        super(AtlasCategoryView, self).currentChanged(*args, **kwargs)
    ...
    def item_clicked(self, idx):
        ...
        if _item:
            self.item_selected.emit(_item.atlas_obj)
        else:
            self.item_selected.emit(None)

    def expandAll(self):
        """Expand all the items in the view"""
        super(AtlasCategoryView, self).expandAll()
    ...
    def right_click_menu(self, position):
        ...
        if self.purgatory_mode:
            ...
            if item.atlas_obj.object_type == ObjectType.WORK:
    ...
    def on_resurrect(self, item):
        ...
        Args:
            item (AtlasWorkItem or AtlasPublishItem):
                The work or publish item to be resurrected.
        """
        state, msg = item.atlas_obj.resurrect()
        if not state:
    ...
    def metadata_pre_checks(dcc_handler, metadata):
        ...
        Args:
            dcc_handler (atlas_manager.dcc_handler.DccHandler):
                The DCC handler object.
            metadata (atlas_manager.object.metadata.Metadata):
                The metadata object.
        Yields:
    ...
    def ingest_here(self, item):
        ...
        Args:
            item (AtlasWorkItem): The work item to be ingested.
        Returns:
            None
        """
        # first check for permissions
        if item.atlas_obj.check_permissions(level=1) == -1:
        ...
        parent_task = item.atlas_obj.parent_task

        # check the dcc for any issues that may prevent saving.
        dcc_issues = item.atlas_obj.guard.dcc_handler.pre_save_issues()
        if dcc_issues:
            self.feedback.pop_info(title="DCC Error", text=dcc_issues, critical=True)
            return
        pre_checks = self.new_version_pre_checks(item.atlas_obj, parent_task.metadata)
        ...
        dialog = NewVersionDialog(work_object=item.atlas_obj, parent=self, ingest=True)
        state = dialog.exec_()
        if state:
    ...
    def open_database_folder(self, item):
        ...
        """
        item.atlas_obj.show_database_folder()
    ...
    def open_scene_folder(self, item):
        ...
        Args:
            item (AtlasWorkItem or AtlasPublishItem):
                The work or publish item to be opened.
        """
        item.atlas_obj.show_project_folder()
    ...
    def copy_scene_path(self, item):
        ...
        """
        item.atlas_obj.copy_path_to_clipboard(item.atlas_obj.get_abs_project_path())
    ...
    def delete_item(self, item):
        ...
        state, msg = item.atlas_obj.check_destroy_permissions()
        if not state:
        ...
        if item.atlas_obj.object_type == ObjectType.WORK:
            are_you_sure = self.feedback.pop_question(
                title="Are you sure?",
                text=f"You are about to delete '{item.atlas_obj.name}' completely.\n\n"
            ...
            if item.atlas_obj.publish.versions:
                are_you_sure = self.feedback.pop_question(
                ...
                if are_you_sure == "cancel":
                    return
        elif item.atlas_obj.object_type == ObjectType.PUBLISH:
            are_you_sure = self.feedback.pop_question(
                title="Are you sure?",
                text=f"You are about to delete '{item.atlas_obj.name}' completely.\n\n"
        ...
        state, msg = item.atlas_obj.destroy()
        if not state:
            self.feedback.pop_info(title="Error", text=msg, critical=True)
            return
...
class AtlasCategoryLayout(QtWidgets.QVBoxLayout):
    """Custom QVBoxLayout for the category layout."""
    mode_changed = QtCore.Signal(int)

    def __init__(self, *args, **kwargs):
        """Initialize the layout."""
        super(AtlasCategoryLayout, self).__init__(*args, **kwargs)
        ...
        self.refresh_btn = AtlasIconButton(icon_name="refresh", circle=True, size=18, icon_size=14)
        header_lay.addWidget(self.refresh_btn)

        self.addWidget(HorizontalSeparator(color=(122, 83, 207)))
        ...
        self.work_tree_view = AtlasCategoryView()
    ...
    def set_task(self, task):
        """Set the task.
        Args:
            task (atlas_manager.object.task.Task): The task object.
        """
        if not task:
            self.clear()
            return
    ...
    def populate_categories(self, categories):
        ...
        Args:
            categories (atlas_manager.objects.category.Category): Category object.
        """
        # clear the layout
