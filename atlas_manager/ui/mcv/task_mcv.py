...
from atlas_manager.objects.guard import Guard
...
class AtlasTaskItem(QtGui.QStandardItem):
    ...
    def __init__(self, task_obj):
        ...
        Args:
            task_obj (atlas_manager.objects.task.Task): Task object
        """
        super(AtlasTaskItem, self).__init__()
...
class AtlasTaskColumnItem(QtGui.QStandardItem):
    def __init__(self, text):
        super(AtlasTaskColumnItem, self).__init__(text)
        self.setEditable(False)


class AtlasTaskModel(QtGui.QStandardItemModel):
    columns = ["name", "id", "path"]
    filter_key = "super"

    def __init__(self):
        """Initialize the model"""
        super(AtlasTaskModel, self).__init__()
    ...
    def append_task(self, task_obj):
        ...
        _task_item = AtlasTaskItem(task_obj)
        pid = AtlasTaskColumnItem(str(task_obj.id))
        path = AtlasTaskColumnItem(task_obj.path)
    ...
    def find_item_by_id_column(self, unique_id):
        ...
        for x in _all_items:
            if isinstance(x, AtlasTaskItem):
                if x.task.id == unique_id:
                    return x


class AtlasTaskView(QtWidgets.QTreeView):
    ...
    def __init__(self):
        """Initialize the view"""
        super(AtlasTaskView, self).__init__()
        self.purgatory_mode = False
    ...
    def currentChanged(self, *args, **kwargs):
        super(AtlasTaskView, self).currentChanged(*args, **kwargs)
    ...
    def expandAll(self):
        ...
        super(AtlasTaskView, self).expandAll()
    ...
    def edit_task(self, item):
        if item.task.check_permissions(level=2) == -1:
            ...
            return
        _dialog = atlas_manager.ui.dialog.task_dialog.EditTask(
...
class AtlasTaskLayout(QtWidgets.QVBoxLayout):
    def __init__(self):
        """Initialize the layout"""
        super(AtlasTaskLayout, self).__init__()
