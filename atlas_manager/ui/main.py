...
import sys
p_path = "D:\\dev\\atlas_manager\\"
if p_path not in sys.path:
    sys.path.append(p_path)

kill_list = []
for name, _module in sys.modules.items():
    if name.startswith("atlas_manager"):
        kill_list.append(name)
for x in kill_list:
    sys.modules.pop(x)

# from atlas_manager.ui.Qt import QtWidgets, QtCore, QtGui
...
import atlas_manager._version as version
from atlas_manager.core.constants import ValidationResult, ValidationState
from atlas_manager import management
from atlas_manager.core import utils
from atlas_manager.management.exceptions import SyncError
from atlas_manager.ui import pick
from atlas_manager.ui.Qt import QtWidgets, QtCore, QtGui
from atlas_manager.ui.dialog.feedback import Feedback, Confirmation
from atlas_manager.ui.dialog.project_dialog import NewProjectDialog, SetProjectAsTemplateDialog
from atlas_manager.ui.dialog.publish_dialog import PublishSceneDialog
from atlas_manager.ui.dialog.settings_dialog import SettingsDialog
from atlas_manager.ui.dialog.update_dialog import UpdateDialog
from atlas_manager.ui.dialog.user_dialog import LoginDialog, NewUserDialog
from atlas_manager.ui.dialog.work_dialog import (
    NewWorkDialog,
...
from atlas_manager.ui.mcv.category_mcv import AtlasCategoryLayout
from atlas_manager.ui.mcv.project_mcv import AtlasProjectWidget
from atlas_manager.ui.mcv.subproject_mcv import AtlasSubProjectLayout
from atlas_manager.ui.mcv.task_mcv import AtlasTaskLayout
from atlas_manager.ui.mcv.user_mcv import AtlasUserWidget
from atlas_manager.ui.mcv.version_mcv import AtlasVersionLayout
from atlas_manager.ui.widgets.common import AtlasButton, VerticalSeparator
from atlas_manager.ui.widgets.pop import WaitDialog

LOG = logging.getLogger(__name__)
WINDOW_NAME = f"Atlas Manager {version.__version__}"


def launch(dcc="Standalone", dont_show=False):
    """Launch the main UI."""
    window_name = f"Atlas Manager {version.__version__} - {dcc}"
    all_widgets = QtWidgets.QApplication.allWidgets()
    atlas = atlas_manager.initialize(dcc)
    if atlas.dcc.custom_launcher:
        return atlas.dcc.launch(atlas, window_name=window_name, dont_show=dont_show)
    parent = atlas.dcc.get_main_window()
    ...
    main_ui_obj = MainUI(atlas, parent=parent, window_name=window_name)
    if not dont_show:
        main_ui_obj.show()
    return main_ui_obj
...
class MainUI(QtWidgets.QMainWindow):
    ...
    def __init__(self, main_object, window_name=WINDOW_NAME, **kwargs):
        ...
        # set style
        _style_file = pick.style_file(file_name="atlasManager.qss")
    ...
    def resume_last_state(self):
        ...
        subproject_id = self.atlas.user.last_subproject
                    ...
                    if state:
                        ...
                        if work_id:
                            ...
                            if state:
                                ...
                                version_id = self.atlas.user.last_version
                                if version_id:
        ...
        self.subprojects_mcv.sub_view.set_expanded_state(
            self.atlas.user.expanded_subprojects
        )
        ...
        _sizes = self.atlas.user.split_sizes or [291, 180, 290, 291]
        self.splitter.setSizes(_sizes)

        self.subprojects_mcv.sub_view.show_columns(
            self.atlas.user.visible_columns.get("subprojects", [])
        )
        self.tasks_mcv.task_view.show_columns(
            self.atlas.user.visible_columns.get("tasks", [])
        )
        self.categories_mcv.work_tree_view.show_columns(
            self.atlas.user.visible_columns.get("categories", [])
        )

        self.subprojects_mcv.sub_view.set_column_sizes(
            self.atlas.user.column_sizes.get("subprojects", {})
        )
        self.tasks_mcv.task_view.set_column_sizes(
            self.atlas.user.column_sizes.get("tasks", {})
        )
        self.categories_mcv.work_tree_view.set_column_sizes(
            self.atlas.user.column_sizes.get("categories", {})
        )

        # set the main window state back
        window_state = self.atlas.user.main_window_state
        if window_state:
            self.setGeometry(QtCore.QRect(*self.atlas.user.main_window_state))

        # enable/disable ui elements
        ui_elements = self.atlas.user.ui_elements
    ...
    def set_last_state(self):
        ...
        self.atlas.user.visible_columns = columns_states
        ...
        self.atlas.user.column_sizes = column_sizes

        self.atlas.user.main_window_state = (
            self.geometry().x(),
            self.geometry().y(),
            self.geometry().width(),
        ...
        self.atlas.user.ui_elements = ui_elements

    def initialize_mcv(self):
        """Initialize the model-control-views."""
        self.project_mcv = AtlasProjectWidget(self.atlas, parent=self)
        ...
        self.user_mcv = AtlasUserWidget(self.atlas.user)
        ...
        self.versions_mcv = AtlasVersionLayout(self.atlas.project, parent=self)
        ...
        if self.atlas.dcc.name == "Standalone":
    ...
    def closeEvent(self, event):  # pylint: disable=invalid-name
        ...
        self.atlas.user.last_subproject = None
        self.atlas.user.last_task = None
        self.atlas.user.last_category = None
        self.atlas.user.last_work = None
        self.atlas.user.last_version = None
        ...
        self.atlas.user.resume.apply_settings()
    ...
    def build_buttons(self):
        "Build the buttons"
        # Work buttons
        save_new_work_btn = AtlasButton("Save New Work")
        save_new_work_btn.setMinimumSize(12, 40)
        work_from_template_btn = AtlasButton("Work from Template")
        work_from_template_btn.setMinimumSize(12, 40)
        save_file_as_work_btn = AtlasButton("Save File as Work")
        save_file_as_work_btn.setMinimumSize(12, 40)
        save_folder_as_work_btn = AtlasButton("Save Folder as Work")
        save_folder_as_work_btn.setMinimumSize(12, 40)
        increment_version_btn = AtlasButton("Increment Version")
        increment_version_btn.setMinimumSize(12, 40)
        self.ingest_version_btn = AtlasButton("Ingest Version")
        self.ingest_version_btn.setMinimumSize(12, 40)
        publish_scene_btn = AtlasButton("Publish Scene")
        publish_scene_btn.setMinimumSize(12, 40)
        publish_snapshot_btn = AtlasButton("Publish Snapshot")
    ...
    def build_bars(self):
        ...
        save_selection_as_work.setEnabled(self.atlas.dcc.save_selection_enabled)
        file_menu.addSeparator()
        ...
        help_menu.addSeparator()
        support_atlas_manager = QtWidgets.QAction("&Support Atlas Manager", self)
        help_menu.addAction(support_atlas_manager)
        ...
        online_docs.triggered.connect(
            lambda: webbrowser.open("https://atlas-manager.readthedocs.io/en/latest/")
        )
        issues_and_feature_requests.triggered.connect(
            lambda: webbrowser.open("https://github.com/masqu3rad3/atlas_manager/issues")
        )
    ...
    def on_purge_local_purgatory(self):
        ...
        if not confirm:
            return
        state, msg = self.atlas.purgatory.purge_local()
    ...
    def on_purge_project_purgatory(self):
        ...
        if not confirm:
            return
        text_confirm_obj = Confirmation(parent=self, confirmation_word=self.atlas.project.name)
        ...
        state, msg = self.atlas.purgatory.purge_origin()
    ...
    def build_extensions(self):
        ...
        if self.atlas.dcc.extensions:
            # create a menu item for the dcc extensions
            dcc_menu = self.menu_bar.addMenu(f"{self.atlas.dcc.name}")
        else:
            return
        for _key, extension_class in self.atlas.dcc.extensions.items():
            extension = extension_class(self)
    ...
    def management_connect(self, platform_name=None):
        ...
        wait_dialog.display()
        handler, msg = self.atlas.get_management_handler(platform_name)
    ...
    def on_settings(self):
        ...
        dialog = SettingsDialog(self.atlas, parent=self)
        dialog.show()
    ...
    def on_publish_scene(self):
        ...
        publish_dialog = PublishSceneDialog(self.atlas.project, parent=self)
    ...
    def _new_work_pre_checks(self, task):
        ...
        dcc_name = self.atlas.project.guard.dcc
        current_dcc_version = self.atlas.dcc.get_dcc_version()
    ...
    def on_work_from_template(self):
        ...
        available_templates = self.atlas.get_template_names()
    ...
    def on_save_any_file(self, file_path=None, folder=False):
        ...
        if not file_path:
            ...
            dialog.setDirectory(self.atlas.project.absolute_path)
    ...
    def _new_version_pre_checks(self, work_obj):
        ...
        for msg in self.categories_mcv.work_tree_view.metadata_pre_checks(
                self.atlas.dcc, work_obj.get_metadata(work_obj.parent_task)
        ):
            yield msg
    ...
    def on_new_version(self):
        ...
        if not self._pre_check(level=1):
            return
        scene_file_path = self.atlas.dcc.get_scene_file()
        if not scene_file_path:
            ...
            return
        work, _version = self.atlas.project.find_work_by_absolute_path(scene_file_path)
    ...
    def on_check_for_updates(self):
        """Check for updates."""
        release_object = self.atlas.get_latest_release()
    ...
    def _can_proceed_without_management(self, platform):
        ...
        ret = self.feedback.pop_question(title=f"Connection Error - {platform}",
                                         ...
                                         buttons=["yes", "no"])
        if ret == "no":
            self.atlas.fallback_to_default_project()
    ...
    def management_lock(self):
        ...
        management platform."""
        is_management_driven = self.atlas.project.settings.get("management_driven", False)
        if is_management_driven:
            ...
            management_platform = self.atlas.project.settings.get("management_platform")
    ...
    def on_create_new_project(self):
        ...
        if not self._pre_check(level=3):
            return
        dialog = NewProjectDialog(self.atlas, parent=self)
        state = dialog.exec_()
        if state:
            self.refresh_project()
    ...
    def on_add_new_user(self):
        ...
        if not self._pre_check(level=3):
            return
        dialog = NewUserDialog(self.atlas.user, parent=self)
        state = dialog.exec_()
        if state:
    ...
    def on_login(self):
        """Launch login dialog."""
        dialog = LoginDialog(self.atlas.user, parent=self)
        dialog.show()
    ...
    def on_show_preview(self):
        ...
        _version = _work_item.atlas_obj.get_version(_version_index)

        executable = self.atlas.user.settings.get("video_player", None)

        preview_dict = _version.previews
        if len(preview_dict.values()) == 1:
            abs_path = _work_item.atlas_obj.get_abs_project_path(
        ...
        for z_key in list(preview_dict.keys()):
            ...
            temp_action.triggered.connect(
                lambda ignore=z_key, item=_work_item.atlas_obj.get_abs_project_path(
                    preview_dict[z_key]
    ...
    def on_element_view(self, element_type, element_path):
        ...
        """
        executable = self.atlas.user.settings.get(f"{element_type}_viewer", None)
    ...
    def on_set_project_as_template(self):
        ...
        if not self._pre_check(level=3):
            return
        dialog = SetProjectAsTemplateDialog(self.atlas, parent=self)
        dialog.show()
    ...
    def _pre_check(self, level):
        ...
        if self.atlas.project.check_permissions(level=level) == -1:
            msg, _type = self.atlas.log.get_last_message()
