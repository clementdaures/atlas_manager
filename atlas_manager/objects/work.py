...
from atlas_manager.dcc.standalone.main import Dcc as StandaloneDcc
from atlas_manager.core.settings import Settings
from atlas_manager.core import filelog
from atlas_manager.objects.publish import Publish
from atlas_manager.objects.version import WorkVersion
...
class Work(Settings, LocalizeMixin):
    ...
    def new_version_from_path(self, file_path, notes=""):
        ...
        abs_version_path = self.get_abs_project_path("edit", version_name) # edits to create the work folder
        ...
        version_dict = {
            ...
            "scene_path": Path("edit", str(version_name)).as_posix(), # edits to create the work folder
    ...
    def new_version(self, file_format=None, notes="", ignore_checks=True, from_selection=False):
        ...
        origin_path = self.get_abs_project_path("edit", version_name) # edits to create the work folder
        thumbnail_path = self.get_abs_database_path("thumbnails", thumbnail_name)
        ...
        output_path = self.get_output_path("edit", version_name) # edits to create the work folder
        if not output_path:
            return -1
        if from_selection:
