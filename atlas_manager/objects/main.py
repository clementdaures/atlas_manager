"""Main Module for the Atlas Manager"""
...
from atlas_manager.core import filelog, settings, utils
from atlas_manager.core.constants import ValidationState, ValidationResult
from atlas_manager.objects import user, project, purgatory
from atlas_manager import dcc
from atlas_manager import management
from atlas_manager.external.packaging.version import Version
...
class Main:
    """Main Atlas Manager class. Handles User and Project related functions."""
    ...
    def __init__(self, common_folder=None):
        ...
        self.default_project = Path(utils.get_home_dir(), "AM_default")

        if not (self.default_project / "_atlas_database" / "project_structure.json").exists():
        ...
        if self.user.get_recent_projects():
            ...
            for _project in reversed(recent_projects):
                if Path(_project, "_atlas_database", "project_structure.json").exists():
                    break
    ...
    def _create_default_project(self):
        ...
        _project_path = Path(utils.get_home_dir(), "AM_default")
        _database_path = _project_path / "_atlas_database"
        ...
        structure_data["name"] = "AM_default"
    ...
    def create_project(
        ...
        **kwargs
        ...
        if not self.user.is_authenticated:
            ...
            return -1
        # database_path = Path(path, "_atlas_database")
        database_path = path_obj / "_atlas_database"
    ...
    def can_set_project(self, absolute_path) -> ValidationResult:
        ...
        if not Path(absolute_path, "_atlas_database", "project_structure.json").exists():
            if self.user.permission_level < 3:
                msg = "The selected folder is not a Atlas Manager project, and you do not have the necessary privileges to set it as one. Action cannot be completed."
                self.log.error(msg)
                return ValidationResult(ValidationState.ERROR, msg, allow_proceed=False)
            msg = "The selected folder is not currently defined as a Atlas Manager project. If you proceed, the necessary database files and folder structure will be created, and this folder will be designated as a Atlas Manager project.\n\nDo you want to continue?"
    ...
    def get_latest_release(self):
        ...
        try:
            conn.request("GET", "/repos/masqu3rad3/atlas_manager/releases/latest", headers=headers)
