...
from atlas_manager.core.constants import ObjectType, ColorCodes, ValidationResult, ValidationState, BranchingModes
from atlas_manager.core.settings import Settings
from atlas_manager.mixins.localize import LocalizeMixin
from atlas_manager.core import filelog

LOG = filelog.Filelog(logname=__name__, filename="atlas_manager")
...
class PublishVersion(Settings, LocalizeMixin):
    ...
    def _get_promoted_folder(self):
        ...
        # resolve the LIVE folder
        promoted_folder = Path(self.get_abs_project_path()).parent / "pro"
    ...
    def _make_live_with_active_branching(self):
        ...
        for element_data in self.elements:
            ...
            # USD Bundled
            if element_data.get("bundled") and element_data.get("bundle_match_id") == 8002:
                bundle_info = element_data.get("bundle_info", {})

                bundle_live_folder = live_folder / f"live_{self._name}"
                bundle_live_folder.mkdir(parents=True, exist_ok=True)

                live_bundle_info = {}
                for ref_name, ref_data in bundle_info.items():
                    versioned_usd = Path(ref_data["path"])
                    live_usda_name = f"{versioned_usd.stem}.usda"
                    live_usda_path = bundle_live_folder / live_usda_name

                    relative_usd = os.path.relpath(
                        versioned_usd, start=bundle_live_folder
                    ).replace("\\", "/")

                    if live_usda_path.exists():
                        utils.write_unprotect(live_usda_path)
                    with open(live_usda_path, "w") as f:
                        f.write(
                            "#usda 1.0\n(\n    subLayers = [\n"
                            f"        @{relative_usd}@\n"
                            "    ]\n)\n"
                        )
                    utils.write_protect(live_usda_path)

                    # store the live .usda path per ref inside bundle_info
                    live_bundle_info[ref_name] = {
                        "extension": ".usda",
                        "path": live_usda_path.relative_to(bundle_live_folder).as_posix(),
                        "sequential": False,
                    }

                # ONE element for the whole bundle, bundle_info carries per-ref live paths
                _data["elements"].append({
                    "name": element_data["name"],       # "USD Bundled"
                    "type": element_data["type"],       # "bun"
                    "suffix": ".usda",
                    "path": bundle_live_folder.relative_to(live_folder).as_posix(),
                    "bundled": True,
                    "bundle_info": live_bundle_info,
            ...
            if publish_path.suffix.startswith(".usd"):
                live_element_name = f"live_{self._name}.usda"
                ...
                with open(live_path, "w") as f:
                    f.write(
                        ...
                        "(\n"
                        "    subLayers = [\n"
                        f"        @{str(publish_path).replace(str(Path(self.get_abs_project_path()).parent), '../')}@\n"
                        "    ]\n"
                        ")\n"
                    )
                ...
                live_element_name = f"live_{self._name}{publish_path.suffix}"
                live_path = live_folder / live_element_name
    ...
    def _promote_with_active_branching(self):
        ...
        for element_data in self.elements:
            ...
            if element_data.get("bundled") and element_data.get("bundle_match_id") == 8002:
                bundle_info = element_data.get("bundle_info", {})
                for ref_name, ref_data in bundle_info.items():
                    versioned_usd = Path(ref_data["path"])           # e.g. .../bun_.../pro_soul_rig.usd
                    live_usda_name = f"live_{versioned_usd.stem}.usda"
                    live_usda_path = promoted_folder / live_usda_name

                    relative_usd = os.path.relpath(versioned_usd, start=promoted_folder).replace("\\", "/")
                    ...
                    _data["elements"].append({
                        ...
                        "type": element_type,
                        "suffix": ".usda",
                        "path": live_usda_path.relative_to(promoted_folder).as_posix(),
                        "bundled": True,
                        "bundle_info": element_data["bundle_info"],
                        "bundle_match_id": element_data["bundle_match_id"],
                    })
                continue
            if publish_path.suffix.startswith(".usd"):
                promoted_element_name = f"pro_{self._name}.usda" # {element_type.upper()}
...
(
    subLayers = [
            ...
            # if it's any other file type (not usd)
                ...
                promoted_element_name = f"pro_{self._name}{publish_path.suffix}" # {element_type.upper()}
                promoted_path = promoted_folder / promoted_element_name
