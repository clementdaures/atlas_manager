...
from atlas_manager.objects.preview import Preview
from atlas_manager.dcc.standalone import main as standalone
from atlas_manager.objects.publish import PublishVersion
...
class Publisher:
    ...
    def reserve(self):
        ...
        self._published_object.add_property(
            "path", Path(self._work_object.path, "pbh").as_posix()
        )
        self._published_object.add_property(
    ...
    def get_management_tasks(self):
        ...
        are not the same as the atlas tasks.
        """
        if not self._work_object:
            LOG.warning("No work object found. Aborting.")
            return []
